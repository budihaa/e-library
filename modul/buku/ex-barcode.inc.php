	...
	*/
	...
	class BARCODE
	{
		...
		function BARCODE($encoding="EAN-13")
		{
		    ...
		    $this->_font=dirname($_SERVER["PATH_TRANSLATED"])."/"."arial.ttf";
			if (isset($_SERVER['WINDIR']) && file_exists($_SERVER['WINDIR']))
			  $this->_font=$_SERVER['WINDIR']."\Fonts\arial.ttf";
		...
		*/
		...
		function _c93Barcode($barnumber,$scale=1,$file="",$checkdigit=false)
		{
			...
			$font_arr=@imagettfbbox ( 1*10, 0, $this->_font, $barnumber);
			$x= floor($total_x-(int)$font_arr[0]-(int)$font_arr[2]+1*10)/2;	
			@imagettftext($im,1*10,0,$x, $height2, $bar_color,$this->_font , $barnumber);
		...
		*/
		...
		function _codaBarcode($barnumber,$scale=1,$file="")
		{
			...
			@imagettftext($im,1*6,0,$x, $height2, $bar_color,$this->_font , $barnumber);
		...
		*/
		...
		function _upceBarcode($barnumber,$scale=1,$file="")
		{
			...
			@imagettftext($im,1*6,0, $space['left'], $height, $bar_color,$this->_font , $encbit);

			
			$x= $space['left']+1*strlen($barnumber)+1*6;	
			@imagettftext($im,1*6,0,$x, $height2, $bar_color,$this->_font , $barnumber);

			$x=$total_x-$space['left']-1*6;
			@imagettftext($im,1*6,0, $x, $height, $bar_color,$this->_font , $checkdigit);
		...
		*/
		...
		function _ean8Barcode($barnumber,$scale=1,$file="")
		{
			...
			$x= $space['left']+1*strlen($barnumber);	
			@imagettftext($im,1*6,0,$x, $height2, $bar_color,$this->_font , $str);
			
			$str=substr($barnumber,4,4);
			$x=$space['left']+1*strlen($bars)/1.65;
			@imagettftext($im,1*6,0, $x, $height2, $bar_color,$this->_font ,$str);
		...
		*/
		...
		function _eanBarcode($barnumber,$scale=1,$file="")
		{
			...
			@imagettftext($im,1*6,0, $space['left'], $height, $bar_color,$this->_font , $str);
			...
			$x= $space['left']+1*strlen($barnumber)+$scale*6;	
			@imagettftext($im,1*6,0,$x, $height2, $bar_color,$this->_font , $str);
			...
			$x=$space['left']+1*strlen($bars)/1.65+$scale*6;
			@imagettftext($im,1*6,0, $x, $height2, $bar_color,$this->_font ,$str);
			...
			if($this->_encode=="UPC-A")
			{
				...
				$x=$total_x-$space['left']-1*6;
				@imagettftext($im,1*6,0, $x, $height, $bar_color,$this->_font , $str);
			}
