...
echo"<h2>Data Anggota</h2><center>
<div style='width:1050px; height:50px; vertical-align:midle; text-align:left'>";
if ($_SESSION['levelpus']=="Ketua") {
echo "<a id='wew' href='?modul=anggota&aksi=tambahanggota' title='Tambah Anggota'/><img style='padding-left:5px' src='images/tambah.png' width='40px'/></a>";
}
echo "<a id='wew' href='?modul=pencarian&aksi=anggota' title='Cari Data Anggota'/><img style='padding-left:5px' src='images/search.png' width='40px'/></a><a id='wew' href='#' onclick=\"window.open('modul/laporananggota/cetak.php', 'WinC',
'width=800,height=400,toolbar=no,location=no,position=center,directories=no,status=no,menubar=no, minimize=yes scrollbars=no,resizable=yes,copyhistory=no,top=50,left=200')\" title='Cetak Data Anggota'/><img style='padding-left:5px' src='images/printer.png' width='40px'/></a>
</div>";
echo"<table id='tabel' style='width:1050px; font-size:11px;'>
...
<td width='15%'>Nama</td>
<td width='8%'>JK</td>
<td width='8%'>Tempat Lahir</td>
<td width='10%'>Tanggal Lahir</td>
<td width='30%'>Alamat</td>
<td width='10%'>Status Bebas Pustaka</td>";
error_reporting(0);
$no=1;
$baris=1;
...
echo"<td>$tampil[nim]</td>";
echo"<td>$tampil[nama]</td>";
echo"<td>$tampil[jenis_kelamin]</td>";
echo"<td>$tampil[tempat_lahir]</td>";
echo"<td>$tampil[tanggal_lahir]</td>";
echo"<td>$tampil[alamat]</td>";
echo"<td>$tampil[stts]</td>";

echo"<td align='center'><a href='#' onclick=\"window.open('modul/anggota/tampilbarcode.php?text=$tampil[no_agt]', 'kartuagt',
'menubar=yes,scrollbars=yes,resizable=yes,width=800,height=400,top=50,left=200')\"><u>Kartu</u></a></td>";
if ($_SESSION['levelpus']=="Ketua") {
echo"<td><a href=?modul=anggota&aksi=editanggota&nis=$tampil[no_agt]>Edit</td>";
echo"<td><a onclick=\"return confirm('Anda Yakin Menghapus Data Ini?')\" href='?modul=anggota&aksi=hapus&id=$tampil[no_agt]'>Hapus</td>";
}
$no++;
$baris++;}
echo"</tr>";
...
//INTERFACE TAMBAH
case "tambahanggota":
$qr	= mysql_query("SELECT MAX(CONCAT(LPAD((RIGHT((no_agt),6)+1),6,'0')))FROM anggota");
$qr2	= mysql_query("SELECT MIN(CONCAT(LPAD((RIGHT((no_agt),6)),6,'0')))FROM anggota");	
...
<tr><td>Nim : </td><td>
<input size='30' type='text' name='nim' maxlength='30'></td></tr>
...
echo"<tr><td colspan='2'><img width='80px' height='90' src='images/anggota/resize.php?src=$row[no_agt].jpg&scale=300&q=200'/></td></tr>";
echo"
...
echo'<tr><td>Nim : </td><td><input maxlength=30 type=text name="nim" value="'.$row['nim'].'"></td></tr>';
...
// AKSI INPUT
case "input":
if (empty($_POST['nim']) or empty($_POST['nama']) or empty($_POST['jenis_kelamin']) or empty($_POST['tempat_lahir']) or empty($_POST['tanggal_lahir']) or empty($_POST['alamat']))
{
...
elseif (file_exists($tujuan)){
	...
	move_uploaded_file($temp,$tujuan);
		$nim = str_replace("'", "''",$_POST['nim']);
		$nama = str_replace("'", "''",$_POST['nama']);
		$tempat_lahir = str_replace("'", "''",$_POST['tempat_lahir']);
		$alamat = str_replace("'", "''",$_POST['alamat']);
		$sql = mysql_query("INSERT INTO anggota VALUES('$_POST[no_agt]','$nim','$nama','$_POST[jenis_kelamin]','$tempat_lahir','$_POST[tanggal_lahir]','$alamat','aktif')"); 		
		if (!$sql)
		{
